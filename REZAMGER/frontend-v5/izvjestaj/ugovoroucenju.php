...
if ($predmet==$debug_predmet) print $k++."redovno sigurno (non-fuzzy) $student<br>";
			...
			if (mysql_num_rows($q250)<1) {
				...
				if ($nisu_izabrali == "da") {
					...
					print "- ".mysql_result($q252,0,0)." - ".($semestar)." - ".mysql_result($q251,0,0)." ".mysql_result($q251,0,1)." (".mysql_result($q251,0,2).")<br>";
				}
			} else {
				...
				while ($r260 = mysql_fetch_row($q260)) {
					...
					if ($ponovac) {
						...
						$slusa_odsjek_ponovac[$r260[0]][$studij]++;
					} else {
						$slusa_redovno_sigurno[$r260[0]]++;
						$slusa_odsjek_sigurno[$r260[0]][$studij]++;
if ($r260[0]==$debug_predmet) print $k++."redovno sigurno (non-fuzzy uou) $student<br>";
					...
					if ($imena == $predmet) {
						...
						print "- ".mysql_result($q268,0,0)." ".mysql_result($q268,0,1)." (".mysql_result($q268,0,2).")<br>";
					}
				}
			}
		}
	...
	if (!$novaag_aktuelna || date("m")>10 || date("m")==10 && date("d")>15 || date("Y")>substr($novaag_naziv,0,4)) continue;
		...
		if ($imaugovor && !$ugovor_ponovac) {
			...
			while ($r50 = mysql_fetch_row($q50)) {
				if (!in_array($r50[0], $polozio_koliziono)) {
					...
					$fali_u_koliziji++;
if ($predmet==$debug_predmet) print "redovno sigurno (uou) $student<br>";
				}
			}
		...
		if ($imaugovor && !$ugovor_ponovac) {
			...
			while ($r50 = mysql_fetch_row($q50)) {
				if (!in_array($r50[0], $polozio_koliziono)) {
					$slusa_redovno_sigurno[$r50[0]]++;
					$fali_u_koliziji++;
if ($predmet==$debug_predmet) print "redovno sigurno (uou ašćare) $student<br>";
				}
			}
...
if ($predmet==$debug_predmet) print "redovno možda (obavezan) $student<br>";
				}
...
if ($predmet==$debug_predmet) print "redovno možda (uou) $student<br>";
					}
