...
function izvjestaj_prijemni_kanton() {
...
if ($_REQUEST['akcija']=="kandidati") {
	...
	if ($ciklus==1)
		$q = myquery("SELECT o.ime, o.prezime, us.opci_uspjeh, us.kljucni_predmeti, us.dodatni_bodovi, us.opci_uspjeh+us.kljucni_predmeti+us.dodatni_bodovi AS ukupno, ss.naziv, pp.rezultat FROM prijemni_prijava as pp, osoba as o, uspjeh_u_srednjoj as us, srednja_skola as ss WHERE pp.prijemni_termin=$termin AND pp.osoba=o.id AND o.id=us.osoba AND us.srednja_skola=ss.id AND pp.izasao=TRUE $uslov $orderby");
	else
		$q = myquery("SELECT o.ime, o.prezime, pcu.opci_uspjeh, pcu.dodatni_bodovi, pcu.opci_uspjeh+pcu.dodatni_bodovi AS ukupno, pcu.fakultet, pp.rezultat FROM prijemni_prijava as pp, osoba as o, prosliciklus_uspjeh as pcu WHERE pp.prijemni_termin=$termin AND pp.osoba=o.id AND o.id=pcu.osoba AND pp.izasao=TRUE $uslov $orderby");

	
	?>
	<table width="" align="center" border="1" cellpadding="1" cellspacing="0" bordercolor="#000000">
	<tr>
	<td width="10" valign="center"><b>R.br.</b></td>
	<td valign="center"><b>Prezime i ime</b></td><? 
	if (!$sakrij_bodove) { 
		?>
		<td width="200" valign="center"><b>Srednja škola</b></td>
		<td width="100" valign="center"><b>Opći uspjeh</b></td>
		<? 
/*if ($ciklus==1) { ?><td width="110"><b>Ključni predmeti</b></td><? } ?>
		...
		<? */
		?>
		<td width="100" valign="center"><b>Bodovi<br>(Matematika)</b></td><?
	}
	?></tr>
	<?
	$brojac = 1;
...
/*		if (!$sakrij_bodove) { 
		...
		?>
			<td align="left"><?=$kandidat[6]?></td>
			<td align="center"><?=$kandidat[2]?></td>
			<td align="center"><?=$kandidat[7]?></td>
		</tr>
		<?
		$brojac++;
	}
