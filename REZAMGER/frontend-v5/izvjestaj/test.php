...
$q30 = myquery("select pk.id, p.naziv, pk.obavezan, pk.studij, pk.semestar, p.id from predmet as p, ponudakursa as pk where pk.predmet=p.id and pk.akademska_godina=6 order by pk.studij, pk.semestar, pk.obavezan desc, p.naziv");

// Ispisujemo samo sumarne podatke
while ($r30 = mysql_fetch_row($q30)) {
	$pk=$r30[0];
	$studij=$r30[3]; $semestar=$r30[4];
	$metapredmet = $r30[5];

	// Zaglavlje tabele
	if ($studij != $oldstudij || $semestar!=$oldsemestar) {
		$q35 = myquery("select naziv from studij where id=$studij order by naziv");

		if ($oldstudij!=0 && $oldsemestar!=0) print "</table>";

		?>
		<p><b><?=mysql_result($q35,0,0)?>, <?=$semestar?>. semestar</b></p>
		<table border="1" cellspacing="0" cellpadding="2">
			<tr>
				...
				<td colspan="5" align="center"><b>2009/2010</b></td>
				<td colspan="5" align="center"><b>2010/2011</b></td>
			</tr>
			<tr>
				<!-- Predmet -->
				<!-- 2005/2006 -->
	...
	// Akademska godina
	for ($akgod=1; $akgod<=6; $akgod++) {

		// Preskacemo zadnji semestar..
		if ($akgod==6 && $semestar%2==0) {
		...
		$q40 = myquery("select ss.student, ss.nacin_studiranja, ss.ponovac from student_predmet as sp, student_studij as ss, ponudakursa as pk where sp.predmet=pk.id and pk.predmet=$metapredmet and pk.studij=$studij and pk.semestar=$semestar and pk.akademska_godina=$akgod and sp.student=ss.student and ss.akademska_godina=$akgod and ss.semestar=$semestar");
