...
global $userid,$user_nastavnik,$user_studentska,$user_siteadmin, $conf_files_path;
...
if (!$user_nastavnik && !$user_studentska && !$user_siteadmin && file_exists($filename)) {
	readfile($filename);
	return;
}
...
$q10 = myquery("select naziv, UNIX_TIMESTAMP(objavljeni_rezultati) from predmet where id=$predmet");
if (mysql_num_rows($q10)<1) {
	zamgerlog("nepoznat predmet $predmet",3); // nivo 3: greska
	biguglyerror("Traženi predmet ne postoji");
	return;
}
$q15 = myquery("select naziv, aktuelna from akademska_godina where id=$ag");
...
$imaintegralni=0;
$ispit_id_array = $ispit_komponenta = array();
