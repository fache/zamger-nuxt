...
//$TEMPLATEfilename = "lib/rss_sablon.html";
...
if (!isset($_REQUEST["buildURL"])) {
  ...
  if (isset($outputCacheTTL) && function_exists("FeedForAll_scripts_readOutputCacheFile") && (($cacheContents = FeedForAll_scripts_readOutputCacheFile($XMLfilename, $TEMPLATEfilename)) !== FALSE)) {
    ...
    if (($template = FeedForAll_scripts_readFile($TEMPLATEfilename, $useFopenURL)) === FALSE) {
      if (!isset($hideErrors)) {
        if ($ReadErrorString == "") {
          echo "Nemoguce otvoriti sablon $TEMPLATEfilename, izlazak\n";
        } else {
          echo "Nemoguce otvoriti sablon $TEMPLATEfilename uz gresku <b>$ReadErrorString</b>, izlazak\n";
        }
      }
      return;
    }
    if (FeedForAll_rss2html_isTemplate($template) === FALSE) {
      if (!isset($hideErrors)) {
        echo "$TEMPLATEfilename nije validan rss2html.php template fajl, izlazak\n";
      }
      return;
    }
    ...
    if (($XML = FeedForAll_scripts_readFile($XMLfilename, $useFopenURL, $allowCachingXMLFiles)) === FALSE) {
      if (!isset($hideErrors)) {
        if ($ReadErrorString == "") {
          echo "Nemoguce otvoriti RSS Feed $XMLfilename, izlazak\n";
        } else {
          echo "Nemoguce otvoriti RSS Feed $XMLfilename sa greskom <b>$ReadErrorString</b>, izlazak\n";
        }
      }
      return;
    }
    ...
    if ($parseResult == 0) {
      ...
      $title = $rss_parser->FeedTitle;
      $description = $rss_parser->FeedDescription;
      if ($stripTagsFromFeed) {
        $title = strip_tags($title);
        $description = strip_tags($description);
      }

      $template = FeedForAll_rss2html_str_replace("~~~FeedXMLFilename~~~", FeedForAll_rss2html_EscapeLink($XMLfilename), $template);
      $template = FeedForAll_rss2html_str_replace("~~~FeedTitle~~~", FeedForAll_rss2html_limitLength($title, $limitFeedTitleLength), $template);
      $template = FeedForAll_rss2html_str_replace("~~~FeedDescription~~~", FeedForAll_rss2html_limitLength($description, $limitFeedDescriptionLength), $template);
      ...
      if ($stripTagsFromFeed) {
        ...
        $description = strip_tags($description);
          ...
          $item = FeedForAll_rss2html_str_replace("~~~ItemTitle~~~", FeedForAll_rss2html_limitLength($title, $limitItemTitleLength), $source);
          $item = FeedForAll_rss2html_str_replace("~~~ItemDescription~~~", FeedForAll_rss2html_limitLength($description, $limitItemDescriptionLength), $item);
